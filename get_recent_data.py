...
year = 2025
latest_race = get_completed_f1_rounds(year)

# Get last RaceID to avoid entering dublicate data
last_race_id = df["RaceID"].iloc[-1]

if int(last_race_id.split("_")[0]) == year and int(last_race_id.split("_")[1]) == latest_race:
   print("Data Already There")
else:
   try:
      # If Data not in DB, Get data from Last entry till latest race completed
      last_race_round = int(last_race_id.split("_")[1])
      for race in range(last_race_round+1 , latest_race+1):
         print(race)
